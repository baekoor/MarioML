...
class MarioTrainer:
    def __init__(self,
        ...
        # 25k steps limit
        self.env = TimeLimit(self.env, max_episode_steps=25000)
        print(f"Applied new TimeLimit with max_episode_steps=25000")
        ...
        try:
            ...
            if self.scaler:
                try:
                    ...
                    print(f"Error in mixed precision training: {e}")
            ...
            print("Optimization interrupted. Saving checkpoint before exit...")
            self.save_checkpoint(final=True)
            raise
        except Exception as e:
            print(f"Error during optimization: {e}")

    def train(self, num_episodes=1000, render=False, save_interval=10, revert_threshold=5, performance_threshold=0.7):
        """Train the agent"""
        print("Starting training loop...")
        episode_distance_logs = []  # Store distance logs for all episodes

        # Initialize variables for model reversion logic
        best_recent_avg = 0
        episodes_below_best = 0
        recent_scores = deque(maxlen=10)  # Track recent performance
        self.final_distances = []  # Add this to track final distances
        ...
        try:
            for episode in range(1, num_episodes + 1):
                ...
                try:
                    while not done:
                        ...
                        try:
                            ...
                            print(
                                "Optimization interrupted. Saving checkpoint before exit...")
                            self.save_checkpoint(final=True)
                            raise
                        except Exception as e:
                            print(f"Error during optimization step: {e}")
                            # Consider if we should continue or break here
                    ...
                    print(
                        f"Episode {episode} interrupted. Saving current progress...")
                    # Save partial progress for this episode
                ...
                # Store final distance for plotting
                final_distance = self.distance_log[-1][1] if self.distance_log else 0
                self.final_distances.append(final_distance)

                # Update recent scores for model reversion logic
                ...
                # Save best model
                if total_reward > self.highest_score:
                    self.highest_score = total_reward
                    self.save_best_model()
                    print(
                        f"New best model saved with highest score: {self.highest_score:.2f}")
                    episodes_below_best = 0

                    # Update best recent average if appropriate
                    if recent_avg > best_recent_avg:
                        best_recent_avg = recent_avg
                else:
                    # Check if performance is declining significantly
                    if len(recent_scores) == recent_scores.maxlen and recent_avg < best_recent_avg * performance_threshold:
                        episodes_below_best += 1
                    else:
                        # Update best recent average if improving
                        if recent_avg > best_recent_avg:
                            best_recent_avg = recent_avg
                            episodes_below_best = 0

                    # If performance has been poor for several episodes, revert to best model
                    if episodes_below_best >= revert_threshold:
                        print(
                            f"Performance declining for {episodes_below_best} episodes.")
                        print(
                            f"Recent avg: {recent_avg:.2f}, Best recent avg: {best_recent_avg:.2f}")
                        print("Reverting to best model...")

                        if self.load_best_model():
                            episodes_below_best = 0
                            print(
                                "Successfully reverted to best model. Continuing training.")
                        else:
                            print(
                                "Could not revert to best model. Continuing with current model.")
            ...
            self.env.close()
            print("Training complete or interrupted. Saving final state...")
            self.save_checkpoint(final=True)
    ...
    def load_best_model(self):
        ...
        try:
            ...
            # Update best score and highest score
            self.best_score = checkpoint.get('best_score', self.best_score)
            self.highest_score = checkpoint.get(
                'highest_score', self.highest_score)

            print(f"Best model loaded from {model_path}")
            print(
                f"Best score: {self.best_score:.2f}, Highest score: {self.highest_score:.2f}")
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        try:
            ...
            # Load highest score if available
            self.highest_score = checkpoint.get('highest_score', 0)
            if 'highest_score' not in checkpoint and self.scores:
                # If loading an old checkpoint, initialize highest_score from max of scores
    ...
    def plot_progress(self, save_path=None):
        ...
        # Plot 2: All max distances
        plt.subplot(2, 2, 2)
        plt.plot(self.max_distances)
        plt.title('Max Distances (All Episodes)')
        plt.xlabel('Episode')
        plt.ylabel('Distance')
        plt.grid(True, alpha=0.3)

        # Plot 3: Distance progression for the last episode if available
        plt.subplot(2, 2, 3)
        if self.distance_log:
        ...
        # Plot 4: Tabular representation of final distances
        plt.subplot(2, 2, 4)
        ax = plt.gca()
        ax.axis('off')  # Turn off axis for table

        # Create data for table - focus on episodes every 25 iterations
        episode_numbers = []
        final_distance_values = []

        for i, distance in enumerate(self.final_distances):
            episode_num = i + 1
            if episode_num % 25 == 0 or episode_num == 1 or episode_num == len(self.final_distances):
                episode_numbers.append(episode_num)
                final_distance_values.append(distance)

        # Add more recent episodes for better visibility
        for i in range(max(1, len(self.final_distances) - 5), len(self.final_distances)):
            episode_num = i + 1
            if episode_num not in episode_numbers:
                episode_numbers.append(episode_num)
                final_distance_values.append(self.final_distances[i])

        # Sort by episode number
        sorted_data = sorted(zip(episode_numbers, final_distance_values))
        episode_numbers, final_distance_values = zip(
            *sorted_data) if sorted_data else ([], [])

        # Create the table
        table_data = [['Episode', 'Final Distance']]
        for ep, dist in zip(episode_numbers, final_distance_values):
            table_data.append([str(ep), f"{dist:.1f}"])
        ...
        # Highlight every 25th episode with a different color
        for i, ep in enumerate(episode_numbers, 1):
            if ep % 25 == 0:
                table[(i, 0)].set_facecolor('#E2EFDA')
                table[(i, 1)].set_facecolor('#E2EFDA')

        plt.title('Final Distances (Selected Episodes)')

        # Adjust layout
        plt.tight_layout()

        # Add a detailed table for milestone episodes (every 25 episodes)
        plt.figure(figsize=(12, 8))
        ax = plt.gca()
        ax.axis('off')

        # Get data for milestone episodes
        milestone_data = [['Episode', 'Final Dist',
                           'Max Dist', 'Score', 'Avg Score']]

        for i in range(len(self.max_distances)):
            episode_num = i + 1
            if episode_num % 25 == 0 or episode_num == 1 or episode_num == len(self.max_distances):
                avg_score = self.avg_scores[i] if i < len(
                    self.avg_scores) else 'N/A'
                final_dist = self.final_distances[i] if i < len(
                    self.final_distances) else 0
                milestone_data.append([
                    str(episode_num),
                    f"{final_dist:.1f}",
                    f"{self.max_distances[i]:.1f}",
                    f"{self.scores[i]:.2f}",
                    f"{avg_score:.2f}" if isinstance(
                        avg_score, (int, float)) else avg_score
                ])

        # Create the detailed table
        milestone_table = ax.table(cellText=milestone_data,
                                   loc='center',
                                   cellLoc='center',
        ...
        milestone_table.set_fontsize(12)
        milestone_table.scale(1.5, 1.8)

        # Highlight table headers
        for i in range(len(milestone_data[0])):
            milestone_table[(0, i)].set_facecolor('#4472C4')
            milestone_table[(0, i)].set_text_props(color='white')

        plt.title(
            'Performance at Milestone Episodes (Every 25 Episodes)', fontsize=16)

        # Create an additional plot to compare max vs final distances
        plt.figure(figsize=(12, 8))
        plt.plot(self.max_distances, label='Max Distance', color='blue')
        plt.plot(self.final_distances, label='Final Distance', color='green')
        plt.title('Distance Comparison: Max vs Final Position')
        plt.xlabel('Episode')
        plt.ylabel('Distance')
        plt.legend()
        plt.grid(True, alpha=0.3)

        # Add markers for milestone episodes
        milestone_episodes = [i for i, ep in enumerate(
            range(1, len(self.max_distances) + 1)) if ep % 25 == 0 or ep == 1]

        # Only add markers if we have data
        if milestone_episodes and len(self.max_distances) > 0:
            max_distances_array = np.array(self.max_distances)
            final_distances_array = np.array(self.final_distances)

            # Handle the case where arrays might have different lengths
            valid_indices = [i for i in milestone_episodes if i < len(
                max_distances_array) and i < len(final_distances_array)]

            if valid_indices:
                plt.plot(
                    valid_indices, max_distances_array[valid_indices], 'ro', markersize=8)
                plt.plot(
                    valid_indices, final_distances_array[valid_indices], 'go', markersize=8)

                # Add episode numbers as text labels for important points
                for idx in valid_indices:
                    episode_num = idx + 1
                    plt.text(
                        idx, max_distances_array[idx] + 5, f"Ep {episode_num}", fontsize=9, ha='center')

        # Set the x-axis to show integer episode numbers
        plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))

        # Save or show all plots
        if save_path:
            # Save main plots
            plt.figure(1)
            plt.savefig(f"{save_path}_main.png")

            # Save milestone table
            plt.figure(2)
            plt.savefig(f"{save_path}_milestone_table.png")

            # Save distance comparison
            plt.figure(3)
            plt.savefig(f"{save_path}_distance_comparison.png")

            print(
                f"Progress plots saved to {save_path}_main.png, {save_path}_milestone_table.png, and {save_path}_distance_comparison.png")
        else:
            plt.show()
...
def main():
    ...
    parser.add_argument("--revert-threshold", type=int, default=5,
                        help="Number of poor episodes before reverting to best model")
    parser.add_argument("--performance-threshold", type=float, default=0.7,
                        help="Fraction of best performance that triggers reversion (0.0-1.0)")
